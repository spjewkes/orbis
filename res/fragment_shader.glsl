...
void main()
{
	...
	vec3 diffuse = texture( Tex_Cube, UV ).rgb * Light_Col * cos_angle / (distance * distance);
	...
	vec3 specular = texture( Tex_Cube, UV ).rgb * Light_Col * pow(cos_alpha, 4) / (distance * distance);
